...
const printHelp = () => {
  console.log(`
Create VMF v3.1.8 Framework Package data

Usage:
  node src/scripts/createVmfV318PackageData.js --source-dir <path> [options]

Options:
  --source-dir <path>       Unpacked VMF v3.1.8 seed-pack directory. Required.
...
const createVmfV318PackageData = async (optionOverrides = {}) => {
  const args = { ...optionOverrides }
  if (!args.seedDir) throw new Error('--source-dir is required.')
  if (args.apply && !args.database) throw new Error('--apply requires an explicit database name.')
  ...
  try {
    ...
    return {
      ...
      script: 'createVmfV318PackageData.js',
      seedVersion: SEED_VERSION,
    }
  } finally {
...
const main = async () => {
  ...
  const result = await createVmfV318PackageData(args)
  const output = {
