...
const intentProjection = (intent = {}) => Object.fromEntries(
  ['outcome', 'decisionPurpose', 'consumer', 'audience', 'requestedOutputTypeKey', 'format', 'channel', 'requirements']
...
const respond = (state, deps, extra = {}) => ({
  ...
  intent: intentProjection(state.intent), execution, ...extra,
})
const planProjection = (result) => ({
  ...
  execution, idempotent: result.idempotent === true,
})
...
export const planOutcomeStudioRequest = async ({ actorUserId, runtimeInstanceId, scopes, payload = {}, deps = {} } = {}) => {
  ...
  if (payload.action === 'RE_RESOLVE') {
    if (!previous || previous.phase !== 'SAVED') throw failure('OUTCOME_PLANNING_PREDECESSOR_REQUIRED')
  ...
  if (state.field === 'requestedOutputTypeKey') {
    ...
    state.intent.requestedOutputTypeKey = resolution.selectedOutputType.key
  } else if (state.field) {
  ...
  state.field = Object.keys(questions).find((field) => state.intent[field] === undefined) || ''
  ...
  const candidate = await buildOutcomeKnowledgeCompositionPlanForRuntime({ actorUserId, scopes,
  ...
  return respond(state, deps, { planStatus: candidate.status, message: 'Confirm these facts to save the plan. Execution will remain blocked.' })
}
...
export const confirmOutcomeStudioRequestPlan = async ({ actorUserId, runtimeInstanceId, scopes, requestId, payload = {}, deps = {} } = {}) => {
  ...
  return respond({ ...state, phase: 'SAVED', planId: result.plan.planId, planVersion: result.plan.planVersion,
    planFingerprint: result.plan.planFingerprint }, deps, { plan: planProjection(result) })
}
