...
// Set SS009_TEST_MONGOD to an installed mongod executable. Existing data is never reused.
...
const executable = process.env.SS009_TEST_MONGOD
if (!executable || !fs.existsSync(executable)) throw new Error('Set SS009_TEST_MONGOD to an existing executable')
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')
const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'ss009-display-'))
...
const database = `ss009_display_${Date.now()}`
const uri = `mongodb://127.0.0.1:${port}/${database}?replicaSet=ss009_display`
const mongod = spawn(executable, ['--port', String(port), '--bind_ip', '127.0.0.1', '--replSet', 'ss009_display', '--dbpath', data, '--logpath', path.join(directory, 'mongod.log')], { windowsHide: true, stdio: 'ignore' })
...
try {
  ...
  await admin.db('admin').command({ replSetInitiate: { _id: 'ss009_display', members: [{ _id: 0, host: `127.0.0.1:${port}` }] } })
  ...
  const test = spawn(process.execPath, ['--experimental-vm-modules', 'node_modules/jest/bin/jest.js', '--runInBand', '--runTestsByPath', 'src/__tests__/frameworkPackageDisplayBinding.integration.test.js'], {
    cwd: root, windowsHide: true, stdio: 'inherit',
    env: { ...process.env, NODE_ENV: 'test', MONGODB_URI: uri, SS009_TEST_MONGODB_URI: uri,
      AUDIT_SIGNATURE_SECRET: 'synthetic-ss009-display-audit-secret' },
  })
